...
test("AutoLocation should use a HistoryLocation instance when pushStates is supported", function() {
  expect(1);
...
test("AutoLocation should use a HashLocation instance when pushStates are not supported, but hashchange events are and the URL is already in the HashLocation format", function() {
  expect(1);
...
test("AutoLocation should use a NoneLocation instance when neither history nor hashchange are supported.", function() {
  expect(1);
...
test("AutoLocation should use an index path (i.e. '/') without any location.hash as OK for HashLocation", function() {
  expect(1);
...
test("AutoLocation should transform the URL for hashchange-only browsers viewing a HistoryLocation-formatted path", function() {
  expect(3);
...
test("AutoLocation should replace the URL for pushState-supported browsers viewing a HashLocation-formatted url", function() {
  expect(2);
...
test("AutoLocation requires any rootURL given to end in a trailing forward slash", function() {
  expect(3);
  var browserLocation = mockBrowserLocation();
...
test("getHistoryPath() should return a normalized, HistoryLocation-supported path", function() {
  expect(3);
...
test("getHashPath() should return a normalized, HashLocation-supported path", function() { expect(3);
